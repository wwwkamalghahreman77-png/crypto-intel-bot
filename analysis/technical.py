...
def fetch_market_chart(coin_id: str, days: int = 60, vs_currency: str = "usd"):
    ...
    try:
        ...
        print(f"[Technical] خطا در دریافت داده قیمتی برای {coin_id}: {e}")
        return None


def analyze_technical(coin_id: str) -> dict:
    df = fetch_market_chart(coin_id, days=200)
    if df is None or len(df) < 50:
        return {
            "available": False,
            "reason": "داده قیمتی کافی برای تحلیل تکنیکال موجود نیست",
            "score": 0,
        }
    ...
    ema100 = EMAIndicator(close=close, window=min(100, len(close) - 1)).ema_indicator().iloc[-1]
    ema200 = None
    if len(close) >= 200:
    ...
    trend = "صعودی" if ema20 > ema50 else "نزولی"
    breakout = current_price >= recent_high * 0.99
    ...
    return {
        ...
        "bollinger_high": round(bb_high, 6),
        "bollinger_low": round(bb_low, 6),
        "current_price": round(current_price, 6),
        "recent_high_30d": round(recent_high, 6),
