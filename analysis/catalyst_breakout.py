...
تشخیص الگوی «جهش کاتالیزوری» شبیه به اتفاقی که برای AKE (Akedo) و BANK (Lorenzo Protocol) افتاد: یک دوره‌ی نسبتا طولانی تثبیت/رنج فشرده، و بعد یک شکست ناگهانی همراه با جهش شدید حجم و رشد سریع قیمت در چند روز اخیر.
...
MIN_HITS_FOR_MATCH = 3


def analyze_catalyst_breakout(
    structure_df,
    min_hits=MIN_HITS_FOR_MATCH
) -> dict:

    if (
        structure_df is None
        or len(structure_df) < 40
    ):
    ...
    if range_pct <= 35:
        ...
        reasons.append(
            f"پیش از این حرکت، "
            f"حدود یک ماه در یک رنج فشرده "
            f"({range_pct}٪) تثبیت بوده"
        )
    ...
    if window_highs:
        ...
        if (
            ...
            reasons.append(
                "قیمت بالای سقف رنج تثبیت شکسته شده"
            )
