...
def process_confluence_signal(signal, formatter):
    ...
    if decision == "SIGNAL":
        ...
        if already_sent(symbol, signal.get("structure_signal") or signal.get("direction", "UNKNOWN"), signal.get("score", 0), signal.get("score", 0)):
            return
...
def job_send_performance_report():
    ...
    from scheduler.performance import format_performance_report

    text = format_performance_report(days=30)
    message_id = send_message(text)
    log_telegram_message("performance_report", "ALL", message_id, preview=text)

    print("[Job] گزارش عملکرد ارسال شد")
...
def job_market_scan():
    ...
    for signal in signals:
        ...
        if "decision" in signal:
            process_confluence_signal(signal, format_market_signal_v2)
            continue

        # سازگاری با فرمت قدیمی signal_detector (اگر هنوز امتیاز ساده تولید می‌کند)
        symbol = signal.get("symbol")
        ...
        if already_sent(symbol, signal.get("structure_signal") or signal.get("type", "UNKNOWN"), signal.get("score", 0), signal.get("score", 0)):
            continue

        mark_sent(symbol, signal.get("structure_signal") or signal.get("type", "UNKNOWN"), signal.get("score", 0), signal.get("score", 0))

        message_id = send_message(format_market_signal_v2(signal))
        log_telegram_message("signal", symbol, message_id, preview=format_market_signal_v2(signal))

        save_active_signal(signal, message_id)
...
def job_futures_scan():
    ...
    for signal in signals:
        process_confluence_signal(signal, format_futures_signal)
...
def job_spot_scan():
    ...
    for signal in signals:
        process_confluence_signal(signal, format_spot_signal)
...
def _save_closed_trade(row, exit_price, close_reason, highest_tp_hit=None):
    ...
    pct = _calc_profit_pct(signal_type, entry, exit_price)

    risk = abs(entry - stop_loss) if stop_loss else 0
    reward = abs(exit_price - entry)
    rr = round(reward / risk, 2) if risk > 0 else None

    opened_dt = _parse_dt(row.get("date_found", ""))
    duration_hours = None
    if opened_dt:
        duration_hours = round((datetime.utcnow() - opened_dt).total_seconds() / 3600, 2)
    ...
    trade = ClosedTrade(
        ...
        date_opened=row.get("date_found", ""),
        date_closed=now_str(),
        duration_hours=duration_hours or 0,
    )

    try:
        db.insert("closed_trades", trade.to_dict())
    except Exception as e:
        print("[ClosedTrades] خطا در ثبت معامله بسته‌شده:", e)


def job_monitor_active_signals():

    from futures.toobit import get_current_price as get_futures_price
    from spot.toobit import get_current_price as get_spot_price
    ...
    for row in rows:
        ...
        if stop_hit and not row.get("hit_stop"):
            ...
            new_msg_id = send_message(text, chat_id=chat_id, reply_to_message_id=msg_id)
            log_telegram_message("stop_hit", symbol, new_msg_id, reply_to=msg_id, preview=text)
            ...
            _save_closed_trade(row, price, close_reason="STOP_LOSS")
        ...
        for tp_key, hit_key, label in targets:
            ...
            if hit:
                ...
                text = f"✅ {label} برای {symbol} خورد!\nسود: {pct}%"
                new_msg_id = send_message(text, chat_id=chat_id, reply_to_message_id=msg_id)
                log_telegram_message("tp_hit", symbol, new_msg_id, reply_to=msg_id, preview=text)
        ...
        if final_hit_now:
            ...
            _save_closed_trade(row, price, close_reason="ALL_TARGETS_HIT", highest_tp_hit=final_label)
